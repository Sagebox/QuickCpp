...
int main()
{
    ...
    win.SetWindowSize(cWinSize,true);

    auto cBitmap = win.CreateBitmap(cWinSize.cx);       // Create bitmap of (Width,1) --> Returns a CSageBitmap object
    ...
    for (int i=0;i<cWinSize.cy;i++)
    {
        ...
        win.DisplayBitmap(0,i,cBitmap);                       // Print the one-line bitmap to the Y row in the window.
    }
