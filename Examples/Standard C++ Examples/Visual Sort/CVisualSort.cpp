...
// ----------------------------------------------------
// CVisualSort.Cpp -- SageBox Visual Sort Demonstration 
// ----------------------------------------------------
//
// *** Sagebox Qiuck C++ Note: The following is exactly the same as the Sagebox version.  See InitWindow() for some more information, as this is 
// *** the only place this set of code changed from the Sagebox version.
//
...
//
// ** Sagebox Quick C++ Note:
//
//    There are 3-4 lines of code that changed to suit Sagebox Quick C++ vs. the original Sagebox version.
//    The Sagebox version could have been written more generically, in which case NO CHANGES at all would be necessary.
// 
//    The changes are basically about creating the window vs. coming in with the window pre-created with the Sagebox Quick C++ 
//    environment.  With Sagebox alone, without the Quick C++ part of it, you must create your own Sagebox instantiation and Window.
//
//    $$ is used in the code to indicate where code has been changed or added.

bool CVisualSort::InitWindow(CWindow * cWindow)
{
    ...
    // $$ the line below is the line in the Sagebox version

    //      m_cWin = &cSageBox.NewWindow(600,100,kSortSize+100,kSortSize/2+100,"SageBox -- Sorting Algorithms Visualization",InnerSize());    

    // $$ The next three lines are added for Sagebox Quick C++. Instead of creating the window, we 
    //    now just assign it and set the size and location as we did in the NewWindow() call above.
    //
    //    As mentioned, we can make this completely generic by just passing in a Window from the Sagebox version.

    m_cWin = cWindow;                                                  

    m_cWin->SetWinLocation(600,100);                                   
    m_cWin->SetWindowSize(kSortSize+100,kSortSize/2+100,true);         
...
bool CVisualSort::main(CWindow * cWindow)
{
...
//    CSageBox cSageBox 
    ...
    stdAssert(InitWindow(cWindow),"InitWindow() failed");      // We don't really use these messages, but we can if we want to.
    stdAssert(InitMem(),"InitMem() failed");                    // "stdNoMsg" can also be used in the place of a message.


    m_bConsoleApp = cWindow->GetSageBox()->isConsoleApp();                         // See DrawGraph() where this is used to print a message differently
