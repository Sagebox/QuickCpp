// -----------------------------------------------
// Sagebox Quick C++ Color Selector Widget Example
// -----------------------------------------------
//
...
int main()
{
    ...
    win.SetWindowSize(800,400);         // Set an arbitrary size...
    win.SetWinLocation(50,50);          // and location. The Window is hidden until used so part is not visible.

    // Start the color selector. With Sagebox CWindow objects, we can just pass the window.
    // With Sagebox Qiuck C++ window, we get the object from win.GetWindow()
    //
    ...
    CColorSelector cColorSelect(win.GetWindow(),870,50,opt::Popup());
    ...
    win.Write("Close Window to end program",Center() | Font("Arial,16")); 
    ...
    while(win.GetEvent() && !cColorSelect.WindowClosed())
    {
        ...
        if (cColorSelect.ValueChanged(rgbColor)) win.Cls(rgbColor);
