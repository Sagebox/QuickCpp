// --------------------------------------------
// Sagebox Quick C++ Color Wheel Widget Example
// --------------------------------------------
//
...
// Note: This is a Windows only program and does not have a Console Mode window or Console Mode option in the project settings. 
//       Typical Sagebox projects have selectable Console Mode and Windows options, but this example was limited to Windows for clarity.
...
int main()
{
    ...
    win.TextWidget(0,0,"Move Color Wheel to change color of Rectangle. Close Window to Exit",Font("Arial,18") | JustBottomCenter() | OffsetY(-10));

    // Create the color wheel and place it at 20,20, passing in the window object.  For Sagebox Window, this is the window object, but
    // since we're passing in the main Sagebox Quick C++ global window, we need to call win.GetWindow() to get the CWindow object
 
    CColorWheelWidget cColorWheel(win.GetWindow(),20,20); 
    ...
    win.DrawRectangle(RectPos,RectSize,cColorWheel.GetRGBValues());
    ...
    while(win.GetEvent())
    {
        ...
        if (cColorWheel.ValueChanged(rgbColor)) win.DrawRectangle(RectPos,RectSize,rgbColor);
    }
}