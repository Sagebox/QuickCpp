#pragma once
#if !defined(__QuickCPP_Sandbox__)

#include "CQuickCpp.h"

#define __QuickCPP_Sandbox__
#define __Sandbox_h__


namespace Sandbox
{
    extern CWindow * MainWin;
    extern CQuickCpp win; 

    // ------------------------------------------------------------------------
    // $Automated_Begin(CScript.h) -- Automated Interface for Quick CPP functions
    // ------------------------------------------------------------------------
    ...
    SIZE GetTextSize(const char * sFont,const char * sText);

    //-->  Original Prototype: bool GetTextSize(const char * sFont,const char * sText,SIZE & Size);

    bool GetTextSize(const char * sFont,const char * sText,SIZE & Size);
    ...
    // and then call DrawBitmap() to display multiple pixels at a time.
    //
    bool DrawPixel(POINT pPoint,DWORD dwColor);
    ...
    // and then call DrawBitmap() to display multiple pixels at a time.
    //
    bool DrawPixel(POINT pPoint,RGBColor_t rgbColor);
    ...
    // and then call DrawBitmap() to display multiple pixels at a time.
    //
    bool DrawPixel(int iX,int iY,RGBColor_t rgbColor);
    ...
    CInputBox & inputbox(const char * sInputboxName);
    ...
    CInputBox & inputbox(int iInputBoxID);
  ...
  // Return a CWindow & for a Window with a Name or an ID. 
    ...
    // a false return. 
    //
    bool DisplayBitmap(POINT pLoc,RawBitmap32_t & stBitmap);
    ...
    // a false return. 
    //
    bool DisplayBitmap(POINT pLoc,int iWidth,int iHeight,unsigned char * sMemory);
    ...
    // See DisplayBitmap() for more details on this function.
    //
    bool DisplayBitmapR(POINT pLoc,int iWidth,int iHeight,unsigned char * sMemory);
    ...
    // See DisplayBitmap() for more details on this function.
    //
    bool DisplayBitmapR(POINT pLoc,RawBitmap_t & stBitmap);
    ...
    // See DisplayBitmap() for more information
    //
    bool DisplayBitmap32(POINT pLoc,int iWidth,int iHeight,unsigned char * sMemory);
    ...
    // See DisplayBitmap() for more information
    //
    bool DisplayBitmap32(POINT pLoc,RawBitmap32_t & stBitmap);
    ...
    // Blend a bitmap with a pre-defined mask
    //
    bool BlendBitmap(POINT pLoc,RawBitmap_t & stBitmap);
    ...
    //
    HFONT AddFont(const char * sFont,const char * sNewFontName = nullptr,unsigned char * ucStatus = nullptr);
   ...
   // MouseClicked() -- returns true if the Left Mouse Button was clicked.
    ...
    /// <returns></returns>
    bool SendWindowEvent(CWindow * cWin = nullptr);
    ...
    // only events are returned and it is not caught in a spining loop. 
    //
    WaitEvent WaitforEvent(cwfEvent & cwEvent);
   ...
   /// <summary>
    ...
    //-->  Original Prototype: [[nodiscard]] RawBitmap32_t GetBitmapStruct32(int iWidth,int iHeight = 1);
    ...
    CInputBox & NewInputBox(int iX,int iY,int iWidth,int iHeight,cwfOpt & cwOpt);
    ...
    CInputBox & NewInputBox(CInputBox * pObject,int iX,int iY,int iWidth,int iHeight,cwfOpt & cwOpt);
    ...
    CInputBox & NewInputBox(int iX,int iY,int iWidth,int iHeight,const char * sText = nullptr,const cwfOpt & cwOpt = cwfOpt());
    ...
    CInputBox & NewInputBox(CInputBox * pObject,int iX,int iY,int iWidth,int iHeight,const char * sText = nullptr,const cwfOpt & cwOpt = cwfOpt());
    ...
    //-->  Original Prototype: [[nodiscard]] CMenu CreateMenu(int iBaseMenuValue = 0);
    ...
    stControlLabel_t AddControlLabel(SizeRect srSize,const char * sText,LabelJust justType,bool bUpdate,const cwfOpt & cwOpt = cwfOpt());
    ...
    bool SetFullScreen(bool bFullscreen = true,bool bForceShow = false);
    ...
    bool DontUpdate(bool bDontUpdate = true,bool bUpdateNow = false);
    ...
    // Note: DevEditBox() and DevInputBox() are the same.  The term Editbox is deprecated and is used for consitency with Windows control names.
    //
    CEditBox & DevInputBox(const char * sInputBoxName = nullptr,const cwfOpt & cwOpt = cwfOpt());
    ...
    bool CreateButtonGroup(const char * sGroupName,int iNumButtons,const wchar_t * * sButtonNames,POINT pLocation,SIZE szSize,int iColumns = 0,SIZE szSpacing = {-1,-1},const cwfOpt & cwOpt = cwfOpt());
    ...
    bool CreateButtonGroup(const char * sGroupName,int iNumButtons,const char * * sButtonNames,POINT pLocation,SIZE szSize,int iColumns = 0,SIZE szSpacing = {-1,-1},const cwfOpt & cwOpt = cwfOpt());
    ...
    bool CreateButtonGroup(const char * sGroupName,int iNumButtons,const char * * sButtonNames,POINT pLocation,int iColumns = 0,const cwfOpt & cwOpt = cwfOpt());
    ...
    // This transfers the visible canvas area only.  It does not copy the window frame or border. 
    //
    bool SendtoClipboard(const CBitmap & cBitmap);
    ...
    /// <param name="sTitle">Name of the Window</param>
    /// <returns>TRUE if Window is valid.  FALSE is the Window is not valid.</returns>
    bool SetWindowTitle(const wchar_t * sTitle);
